...
CMissileCheckPlayerCollisionBehavior::CMissileCheckPlayerCollisionBehavior()
{
}
...
void
CMissileCheckPlayerCollisionBehavior::Update(float dt_)
	...
	bool collision = CCollisionManager::Instance()->GetCollision(myCollider, hit);
		...
		if(collision)
		{
			...
			if (collision)
			{
				...
				{
					//we hit the camera
					OutputDebugStringA("$$$$$$$$$$$$I EAT SHIT$$$$$$$$$$$$$$");

				}*/
				if (strcmp(rendWeHit->GetType(), "CSprite") == 0)
				{
					//we hit the camera
				...
				else
				{
					...
					pExplosion->GetMaterial()->SetTexture(17);//set explosion texture
